...
class AuditSweeper < ActionController::Caching::Sweeper #:nodoc:
  ...
  def after_create(record)
    record.send(:audit_create, current_user, current_ip, current_url, current_referer)
  end

  def after_destroy(record)
    record.send(:audit_destroy, current_user, current_ip, current_url, current_referer)
  end

  def before_update(record)
    record.send(:audit_update, current_user, current_ip, current_url, current_referer)
  end
