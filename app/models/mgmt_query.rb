class MgmtQuery < ActiveRecord::Base
  ...
  def MgmtQuery.job_detect_disconnected_users
    ...
    reconnected = Device.find(:all,
                              :conditions => "(id in (select device_id from access_mode_statuses where mode = 'ethernet') OR id not in (select device_id from access_mode_statuses))" <<
                              ' AND id in (select device_id from gateway_offline_alerts where reconnected_at is null and device_id in ' <<
                              " (select id from device_latest_queries where updated_at > now() - interval '#{ethernet_system_timeout.gateway_offline_timeout_sec*(1+GATEWAY_OFFLINE_TIMEOUT_MARGIN)} seconds'))")
    ...
    reconnected = Device.find(:all,
                              ...
                              ' AND id in (select device_id from gateway_offline_alerts where reconnected_at is null and device_id in ' <<
                              " (select id from device_latest_queries where updated_at > now() - interval '#{dialup_system_timeout.gateway_offline_timeout_sec*(1+GATEWAY_OFFLINE_TIMEOUT_MARGIN)} seconds'))")
    reconnected.each do |device|
      Device.transaction do
        GatewayOnlineAlert.create(:device => device)
    ...
    devices = Device.find(:all,
                          :conditions => "(id in (select device_id from access_mode_statuses where mode = 'ethernet') OR id not in (select device_id from access_mode_statuses))" <<
                          " AND id in (select dlq.id from device_latest_queries dlq where dlq.updated_at < now() - interval '#{ethernet_system_timeout.gateway_offline_timeout_sec*(1+GATEWAY_OFFLINE_TIMEOUT_MARGIN)} seconds')")
    devices.each do |device|
      begin
        MgmtQuery.process_alert(device)
      rescue Exception => e
    ...
    devices = Device.find(:all,
                          :conditions => "id in (select device_id from access_mode_statuses where mode = 'dialup') " <<
                          " AND id in (select dlq.id from device_latest_queries dlq where dlq.updated_at < now() - interval '#{dialup_system_timeout.gateway_offline_timeout_sec*(1+GATEWAY_OFFLINE_TIMEOUT_MARGIN)} seconds')")
    devices.each do |device|
      begin
        MgmtQuery.process_alert(device)
      rescue Exception => e
