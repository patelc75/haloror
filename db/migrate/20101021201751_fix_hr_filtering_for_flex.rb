class FixHrFilteringForFlex < ActiveRecord::Migration
  def self.up
    ddl = <<-eos
    ...
    begin
      ...
      ' (SELECT AVG(heartrate) AS heart FROM vitals WHERE timestamp >= ' || quote_literal(ct) || ' AND timestamp < ' || quote_literal(ct) || ' AND user_id = ' || p_user_id || ' AND heartrate <> -1) a, ' ||
      ' (SELECT AVG(activity) AS act FROM vitals WHERE timestamp >= ' || quote_literal(ct) || ' AND timestamp < ' || quote_literal(ct) || ' AND user_id = ' || p_user_id || ' AND activity <> -1) b, ' ||
      ' (SELECT AVG(hrv) AS avghrv, AVG(orientation) AS avgor FROM vitals WHERE timestamp >= ' || quote_literal(ct) || ' AND timestamp < ' || quote_literal(ct) || ' AND user_id = ' || p_user_id || ' AND (activity <> -1 OR heartrate <> -1)) c ';
      --
      -- old query
      --
