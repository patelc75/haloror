class UpdateDeviceLatestQueriesTrigger < ActiveRecord::Migration
    def self.up
      ddl = <<-eos
  ...
  begin
    ...
    if NOT FOUND then
      insert into device_latest_queries (id, updated_at, reconnected_at) values (new.device_id, v_timestamp, now());
    end if;
    return null;
  end;
  $$ language plpgsql;
